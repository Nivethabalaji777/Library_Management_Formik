...
function LibrarianBookEdit() {
    ...
    let getBookDetails = async () => {
        try {
            setLoading(true);
            const bookDetails = await axios.get(`https://63c90776320a0c4c953f09db.mockapi.io/Books/${params.id}`);
    ...
    const myFormik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                setLoading(true);
                await axios.put(`https://63c90776320a0c4c953f09db.mockapi.io/Books/${params.id}`, values);
                setLoading(false);
